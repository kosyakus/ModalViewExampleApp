...
extension UIColor {
    var hexString: String {
        ...
        let rColor = colorRef?[0] ?? 0
        let gColor = colorRef?[1] ?? 0
        let bColor = ((colorRef?.count ?? 0) > 2 ? colorRef?[2] : gColor) ?? 0
        let aColor = cgColor.alpha

        var color = String(
            format: "#%02lX%02lX%02lX",
            lroundf(Float(rColor * 255)),
            lroundf(Float(gColor * 255)),
            lroundf(Float(bColor * 255))
        )

        if aColor < 1 {
            color += String(format: "%02lX", lroundf(Float(aColor * 255)))
        }
    ...
    public convenience init(hex: String?) {
        ...
        Scanner(string: normalizedHexString).scanHexInt64(&hexInt64)
        self.init(red: UIColorMasks.redValue(hexInt64), green: UIColorMasks.greenValue(hexInt64), blue: UIColorMasks.blueValue(hexInt64), alpha: UIColorMasks.alphaValue(hexInt64))
    }
